...
class GetInfo(CommandBase):
    ...
    def execute(self, params):
        ...
        for npc in npcs_model.npcs:
            ...
            if not npc_model:
                continue
            ret_data["npcs"].append({"id": npc_model.get_token(), "name": npc_model.name, "bio": npc_model.bio, "goal": npc_model.goal, "cash": npc_model.cash})
        
        buildings_model = self.get_single_model("Buildings", create=False)
        if buildings_model:
            for building in buildings_model.buildings:
                print("mayor check building:", {"name": building["n"], "x": building["x"], "y": building["y"], "income": building["eI"], "beds": building["lC"], "livings": building["lL"], "id": building["id"]})
                ret_data["buildings"].append({"name": building["n"], "x": building["x"], "y": building["y"], "income": building["eI"], "beds": building["lC"], "livings": building["lL"], "id": building["id"]})
